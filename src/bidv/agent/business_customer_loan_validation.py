...
from src.bidv.agent.documentation import documentation
from src.bidv.agent.lending_prompt import INCOMING_QUESTION_ANALYSIS
...
"""
{
  ...
  "analysis_type": "overall/trending/explaination_deep_analysis",
  "time_period": "1/2/3"
}
"""
...
class BusinessLoanValidationGraphProvider(GraphProvider[BusinessLoanValidationState]):
    ...
    def __init__(self):
        ...
        self.structured_llm = self.llm.with_structured_output(
            ...
            method="json_mode"  # Hoặc "function_calling" tùy LLM
        )
    ...
    def get_orchestration_information(self, question):
        ...
        return rag_chain.invoke(question).model_dump_json(indent=2)
...
# def get_orchestration_information(question):
...
#             {"question": RunnablePassthrough()}
#             | prompt_template
#             | llm.with_structured_output(OrchestrationInformation, method="json_mode")
#     )
#     return rag_chain.invoke(question)
#
#
# output = get_orchestration_information(
#     "Doanh thu và lợi nhuận của Công ty Cổ phần Chứng khoán DNSE thay đổi ra sao trong giai đoạn từ năm 2022 đến quý I năm 2024")
...
def calculate_financial_metrics(data):
    ...
    for i, year in enumerate(years):
        ...
        # Debt management
        debt_to_equity = total_liabilities / owners_equity if owners_equity and owners_equity != 0 else None
        leverage_ratio = total_assets / owners_equity if owners_equity and owners_equity != 0 else None
        debt_ratio = total_liabilities / total_assets if total_assets and total_assets != 0 else None
        long_term_debt_to_equity = long_term_liabilities / owners_equity if long_term_liabilities and owners_equity and owners_equity != 0 else None

        # Interest coverage ratio = EBIT / Interest Expense
        ebit = operating_profit + interest_expense_borrowings if operating_profit and interest_expense_borrowings else None
        interest_coverage_ratio = ebit / interest_expense_borrowings if ebit and interest_expense_borrowings and interest_expense_borrowings != 0 else None
        ...
        # Growth metrics
        asset_growth_rate = ((
                                         total_assets - prev_total_assets) / prev_total_assets) if prev_total_assets and prev_total_assets != 0 else None
        net_profit_growth_rate = ((
                                              net_profit_after_tax - prev_net_profit) / prev_net_profit) if prev_net_profit and prev_net_profit != 0 else None

        # Asset turnover metrics
        receivables_turnover = total_operating_revenue / receivables if total_operating_revenue and receivables and receivables != 0 else None

        # Operational efficiency (sử dụng total assets năm trước)
        ato = total_operating_revenue / prev_total_assets if total_operating_revenue and prev_total_assets and prev_total_assets != 0 else None
        fixed_asset_turnover = None  # Cần fixed assets value riêng

        # Profit metrics
        ebitda = ebit  # Giả sử EBITDA = EBIT (không có depreciation riêng)
        ebit_margin = ebit / total_operating_revenue if ebit and total_operating_revenue and total_operating_revenue != 0 else None

        # Profitability ratios (sử dụng assets/equity năm trước)
        ros = net_profit_after_tax / total_operating_revenue if net_profit_after_tax and total_operating_revenue and total_operating_revenue != 0 else None
        roa = net_profit_after_tax / prev_total_assets if net_profit_after_tax and prev_total_assets and prev_total_assets != 0 else None
        roe = net_profit_after_tax / prev_owners_equity if net_profit_after_tax and prev_owners_equity and prev_owners_equity != 0 else None

        # Gross profit margin - không có COGS
        gross_profit_margin = None

        # Liquidity ratios
        current_ratio = current_assets / short_term_liabilities if current_assets and short_term_liabilities and short_term_liabilities != 0 else None
        quick_ratio = (
                                  current_assets - 0) / short_term_liabilities if current_assets and short_term_liabilities and short_term_liabilities != 0 else None
        cash_ratio = cash_and_equivalents / short_term_liabilities if cash_and_equivalents and short_term_liabilities and short_term_liabilities != 0 else None
        working_capital = current_assets - short_term_liabilities if current_assets and short_term_liabilities else None
...
# # Sử dụng
# # Giả sử data đã là list of dictionaries
#
# raw_data = json.loads(documentation)  # Your array of dictionaries here
#
# output = calculate_financial_metrics(raw_data)
#
# # In kết quả
# import json
#
