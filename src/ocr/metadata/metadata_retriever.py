...
class SecuritiesFinancialReportMetadataRetriever(DocumentationMetadataRetriever):
    ...
    def get_report_date(self, report_period: str):
        if len(report_period) == 4:
            return f"{report_period}-12-31"
        else:
            quarter = report_period.split("_")[0]
            year = report_period.split("_")[1]
            if quarter.upper() == "Q1":
                return f"{year}-03-31"
        ...
        for execution_output in raw_page_contents:
            if (
                ...
                return {
                    ...
                    "table_of_contents": execution_output.output_content.get(
                        "table_of_contents"
                    ),
                }
