...
class DataValidatorService:
    ...
    def _get_customer_profile(self, business_code: str) -> Optional[CustomerProfile]:
        ...
        try:
            customer = (
                ...
                .filter(CustomerProfile.ma_so_doanh_nghiep == business_code)
                .first()
            )
            return customer
        finally:
    ...
    def _get_legal_representative(
        self, business_code: str, id_number: str
    ) -> Optional[LegalRepresentative]:
        """Get legal representative from database using ORM."""
        session = self.Session()
        try:
            legal_rep = (
                session.query(LegalRepresentative)
                .join(
                    CustomerProfile,
                    LegalRepresentative.customer_id == CustomerProfile.customer_id,
                )
                .filter(
    ...
    def _validate_field(
        ...
        is_consistent_across_doc = doc_consistency.get(field_name, {}).get(
            "is_consistent", True
        )
    ...
    def _get_field_origin_docs(
        self, field_name: str, data: Dict[str, Any]
    ) -> List[OriginDoc]:
        ...
        for key, path_value_map in self.key_value_map.items():
            if len(path_value_map) > 1:
                ...
                if len(unique_values) > 1:
                    contained_documents = list(
                        set(path.split(".")[0] for path in path_value_map.keys())
                    )
